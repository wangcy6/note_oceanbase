...
int ObBackupDataTabletToLSInfo::assign(const ObBackupDataTabletToLSInfo &that)
{
  ...
  if (!that.is_valid()) {
    ...
    LOG_WARN("fail to assign tabelt id", K(ret), K(that));
  } else {
    ls_id_ = that.ls_id_;
  }
  return ret;
...
int ObBackupDataStore::write_tablet_to_ls_info(const ObBackupDataTabletToLSDesc &tablet_to_ls_info, const int64_t turn_id)
{
  int ret = OB_SUCCESS;
  ObBackupPathString full_path;
  share::ObBackupPath path;
  ...
  if (!is_init()) {
    ret = OB_NOT_INIT;
    LOG_WARN("backup data extern mgr not init", K(ret));
  } else if (OB_FAIL(ObBackupPathUtil::get_backup_data_tablet_ls_info_path(backup_set_dest_, turn_id, path))) {
    LOG_WARN("fail to get path", K(ret));
  } else if (OB_FAIL(full_path.assign(path.get_obstr()))) {
    LOG_WARN("fail to assign full path", K(ret));
...
int ObBackupDataStore::write_tablet_to_ls_info(const ObBackupDataTabletToLSDesc &tablet_to_ls_info, const ObLSID &ls_id, 
    const int64_t turn_id, const int64_t retry_id)
{
  ...
  if (!is_init()) {
    ret = OB_NOT_INIT;
    LOG_WARN("backup data extern mgr not init", K(ret));
  } else if (OB_FAIL(ObBackupPathUtil::get_ls_data_tablet_info_path(backup_set_dest_, ls_id, turn_id, retry_id, path))) {
    LOG_WARN("fail to get path", K(ret));
  } else if (OB_FAIL(full_path.assign(path.get_obstr()))) {
    LOG_WARN("fail to assign full path", K(ret));
  } else if (OB_FAIL(write_single_file(full_path, tablet_to_ls_info))) {
...
int ObBackupDataStore::read_tablet_to_ls_info(
    ...
    ObIArray<ObTabletID> &tablet_ids)
{
  int ret = OB_SUCCESS;
  tablet_ids.reset();
  HEAP_VAR(ObBackupDataTabletToLSDesc, tablet_to_ls_info) {
    if (OB_FAIL(read_tablet_to_ls_info(turn_id, tablet_to_ls_info))) {
      ...
      for (int i = 0; OB_SUCC(ret) && i < tablet_to_ls_info.tablet_to_ls_.count(); ++i) {
        ...
        } else if (info.ls_id_ == ls_id && OB_FAIL(append(tablet_ids, info.tablet_id_list_))) {
...
int ObBackupDataStore::read_tablet_to_ls_info(const int64_t turn_id, ObBackupDataTabletToLSDesc &tablet_to_ls_info)
{
  int ret = OB_SUCCESS;
  share::ObBackupPath path;
  ObBackupPathString full_path;
...
int ObBackupDataStore::read_tablet_to_ls_info(const ObLSID &ls_id, const int64_t turn_id, const int64_t retry_id, 
    ...
    int64_t &cur_max_backup_set_id, SCN &restore_start_scn)
{
  ...
  // TODO:  fix this when support inc backup set and full backup set can have different backup dest in 4.1
...
int ObBackupDataStore::get_max_sys_ls_retry_id(const share::ObBackupPath &backup_path,
    const ObLSID &ls_id, int64_t &retry_id)
{
  int ret = OB_SUCCESS;
  retry_id = -1;
  ObBackupIoAdapter util;
  ObArray<ObIODirentEntry> d_entrys;
  const char sys_data_prefix[OB_BACKUP_DIR_PREFIX_LENGTH] = "sys_data_turn_1_retry_";
