...
int ObExprSTArea::eval_st_area(const ObExpr &expr, ObEvalCtx &ctx, ObDatum &res)
{
  ...
  ObExpr *gis_arg = expr.args_[0];//首先从输入参数中提取函数的参数 
  ObEvalCtx::TempAllocGuard tmp_alloc_g(ctx);
  common::ObArenaAllocator &temp_allocator = tmp_alloc_g.get_allocator();
  ObObjType input_type = gis_arg->datum_meta_.type_; //获取参数的数据类型

  if (OB_FAIL(gis_arg->eval(ctx, gis_datum)))  //提取几何图形数据
    LOG_WARN("eval geo arg failed", K(ret)); 
  } else if (gis_datum->is_null()) { //如果参数为 NULL，则将输出结果设为 NULL
    res.set_null();
  } else {
    ObGeometry *geo = NULL;//构建一个 ObGeometry对象
    ...
    if (OB_FAIL(ObTextStringHelper::read_real_string_data(temp_allocator, *gis_datum,
	  ...
	  //构建好的 ObGeometry 对象添加到一个 ObGeoEvalCtx 对象中
